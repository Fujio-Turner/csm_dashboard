...
| Status | `GET /api/status` may be memoized a few seconds in-process. Writes to settings/keys must invalidate. Includes `operator.persona` / `operator.intent` and `personas` (presets from `ai/prompts/operator_persona.json`). |
| People list | `GET /api/people` — `account_id` is **optional**. Omit to list every book (Home chat `/people` and `@`). |
